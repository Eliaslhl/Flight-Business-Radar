...
| `@fbr/flight-providers` | interface `FlightProvider`, `ProviderRegistry` (exécution parallèle isolée), `MockFlightProvider` (7 scénarios), `FixtureFlightProvider`, `FastFlightsProvider` (sidecar), **`TravelpayoutsProvider` (gratuit, données en cache)**, **`SerpApiFlightProvider` (payant)**, **`DuffelFlightProvider` (payant — oracle de confirmation)** + contrats Zod + résolution codes IATA | `@fbr/flight-domain`, `@fbr/shared`, `zod` |
...
| `@fbr/worker` | Process de fond   | Scheduler + worker BullMQ : providers (`SerpApiFlightProvider` si `SERPAPI_API_KEY`, `TravelpayoutsProvider` si `TRAVELPAYOUTS_TOKEN` — données en cache, dédup snapshots par `found_at` —, `FastFlightsProvider` si `FAST_FLIGHTS_URL`, sinon `MockFlightProvider` ; **mode Radar → fan-out sur une tranche rotative de la liste seed**) → journal `provider_requests` → normalizer → `price_snapshots` (append-only, FX) → `analyzeOffers` → `runAlertPipeline` (confirmation des baisses via l'**oracle Duffel** si `DUFFEL_API_TOKEN`, sinon re-requête des providers) → replanification. |

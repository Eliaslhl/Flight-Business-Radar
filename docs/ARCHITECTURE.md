...
| `@fbr/flight-providers` | interface `FlightProvider`, `ProviderRegistry` (exécution parallèle isolée), `MockFlightProvider` (7 scénarios), `FixtureFlightProvider`, `FastFlightsProvider` (sidecar), **`SerpApiFlightProvider` (payant)**, **`DuffelFlightProvider` (payant — oracle de confirmation)** + contrats Zod + résolution codes IATA | `@fbr/flight-domain`, `@fbr/shared`, `zod` |
